clear path
VTOLParamHW07;  % load parameters

% instantiate VTOL, controller, and reference input classes 
VTOL = VTOLDynamics(P);  
controller = VTOLControllerHW07(P);  
z_reference = signalGenerator(2.5, 0.02, 3);  
h_reference = signalGenerator(3, 0.03, 5); 
F_disturbance = signalGenerator(0.0, 0); % if you make this non-zero, the controller won't work well.
